...
int[] image_pixel_symbol_no = new int[pixel_no];
...
void setup(){
  ...
  for (int y = 0; y < (600/pixel_size); y++){
    for (int x = 0; x < (600/pixel_size); x++){
      ...
      image_pixel_symbol_no[this_pixel] = int(map(brightness(c), 0, 255, 0, (symbol_no-1)));
    }
  }
}
...
void draw(){
  ...
  else if (button_pressed == "i") scale(2);
  else if (button_pressed == "o") scale(0.5);
  
  // Convert to ascii and print letters
  for (int y = 0; y < (600/pixel_size); y++){
    for (int x = 0; x < (600/pixel_size); x++){
      int this_pixel = ((600/pixel_size)*y)+x;
      text(ascii_symbols[image_pixel_symbol_no[this_pixel]], x*(pixel_size), y*(pixel_size));
    }
  }
}
